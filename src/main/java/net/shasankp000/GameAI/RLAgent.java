...
public class RLAgent {
    ...
    public int calculateReward(int botX, int botY, int botZ, List<EntityDetails> nearbyEntities, List<String> nearbyBlocks, double distanceToHostileEntity, int botHealth,
        ...
        boolean hasWoolItems = hotBarItems.stream()
                .anyMatch(item -> item.getItem().getName(item).getString().toLowerCase().contains("wool") || item.getItem().getName(item).getString().toLowerCase().contains("carpet"));
        ...
        if (!hostileEntities.isEmpty() && selectedItem.contains("Sword") || selectedItem.contains("Bow") || selectedItem.contains("Axe") || selectedItem.contains("Crossbow") || selectedItem.contains("Trident") && offhandItem.getItem().getName(offhandItem).getString().equalsIgnoreCase("shield")) {
            reward += 20; // Weapon and shield equipped
        } else if (!hostileEntities.isEmpty() && selectedItem.contains("Pickaxe") || selectedItem.contains("Hoe") && offhandItem.getItem().getName(offhandItem).getString().equalsIgnoreCase("shield")) {
            reward += 15; // lower value weapon and shield equipped
        } else if (!hostileEntities.isEmpty() && selectedItem.contains("Air") && offhandItem.getItem().getName(offhandItem).getString().equalsIgnoreCase("shield")) {
