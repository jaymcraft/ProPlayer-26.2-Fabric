...
public class WeaponUtils {
    ...
    private static String getWeaponType(Item item) {
        if (item.getDefaultInstance().is(net.minecraft.tags.ItemTags.SWORDS))
            return "Sword";
        ...
        if (item instanceof MaceItem)
            return "Mace";
        if (item.getDefaultInstance().is(net.minecraft.tags.ItemTags.PICKAXES))
            return "Pickaxe";
    ...
    private static double getAttackDamage(ItemStack stack) {
        ...
        if (item.getDefaultInstance().is(net.minecraft.tags.ItemTags.SWORDS)) {
            baseDamage = 5.0; // Approximation for tools
        } else if (item.getDefaultInstance().is(net.minecraft.tags.ItemTags.SWORDS)) {
            baseDamage = 7.0; // Approximation for swords
    ...
    private static double getAttackSpeed(ItemStack stack) {
        ...
        if (item.getDefaultInstance().is(net.minecraft.tags.ItemTags.SWORDS)) {
            ...
            // Material checks can be approximated or skipped, assuming basic speed
            if (item.toString().contains("wooden") || item.toString().contains("golden")) {
                attackSpeed = 0.8;
            } else if (item.toString().contains("stone")) {
                attackSpeed = 0.8;
            } else if (item.toString().contains("iron")) {
                attackSpeed = 0.9;
            } else if (item.toString().contains("diamond") || item.toString().contains("netherite")) {
                attackSpeed = 1.0;
            }
        } else if (item instanceof TridentItem) {
     ...
     * For simplicity, we use item rarity/material as a proxy for enchantment
     * quality
     */
    private static int analyzeEnchantments(ItemStack stack) {
        int score = 0;

        // Use material name as proxy for enchantment quality
        if (stack.getItem().toString().contains("netherite")) {
            score += 10;
        } else if (stack.getItem().toString().contains("diamond")) {
            score += 8;
        } else if (stack.getItem().toString().contains("iron")) {
            score += 5;
        } else if (stack.getItem().toString().contains("gold")) {
            score += 4;
        }
    ...
    public static int calculateOptimalDrawTime(double distanceToTarget) {
        ...
        if (distanceToTarget <= 7.0) {
            ...
            LOGGER.debug("⚡ Close quarters rapid fire: {} ticks for {}m", drawTime,
            ...
            LOGGER.debug("🎯 Medium range balanced shot: {} ticks for {}m", drawTime,
                    String.format("%.1f", distanceToTarget));
            ...
            LOGGER.debug("🏹 Long range full draw: {} ticks for {}m", drawTime,
                    String.format("%.1f", distanceToTarget));
        } else {
            // VERY LONG RANGE: Maximum draw (125% draw)
            drawTime = 25;
            LOGGER.debug("🎯 Sniper mode max draw: {} ticks for {}m", drawTime,
                    String.format("%.1f", distanceToTarget));
        }
